...
subplot(2,3,5)
for i = 1:length(q_values)
    P_B1_A1 = (q_values(i).*p_range)./(q_values(i).*p_range + (1-q_values(i)).*(1-p_range));
    P_B2_A1 = 1 - P_B1_A1;
...
subplot(2,3,6)
for i = 1:length(q_values)
    P_B1_A2 = ((1-q_values(i)).*p_range)./((1-q_values(i)).*p_range + q_values(i).*(1-p_range));
    P_B2_A2 = 1 - P_B1_A2;
...
% ENHANCED PLOT FORMATTING EXAMPLE
subplot(2,3,1)
set(gca, 'FontSize', 10, 'LineWidth', 1.2)
xlabel('Channel Error Rate (q)', 'FontWeight', 'bold')
ylabel('P[A1]', 'FontWeight', 'bold')
title('Probability of Receiving 0 vs Channel Quality', 'FontSize', 12)

% ADD THRESHOLD CALCULATION
threshold_p = q_values./(q_values + (1-q_values));  % Where P[B1|A1] = P[B2|A1]
