...
% NEW: Display detailed bin metrics for both datasets
display_bin_metrics(results_A, 'Dataset A');
display_bin_metrics(results_B, 'Dataset B');
...
function [results_A, results_B] = binned_analysis(u_A, P_A, u_B, P_B, ...
    ...
    xlim([0 cut_out]); % Fixed syntax error - removed ellipsis
...
function display_bin_metrics(results, dataset_name)
    % NEW FUNCTION: Display detailed bin metrics for each dataset
    fprintf('\n=== Detailed Bin Metrics: %s ===\n', dataset_name);
    fprintf('Bin Range | Mean Wind Speed | Mean Energy | Std Dev | 95%% CI ± | Samples\n');
    fprintf('----------|-----------------|-------------|---------|----------|--------\n');
    
    for b = 1:length(results.mean_speed)
        if ~isnan(results.mean_speed(b))
            bin_start = b-1;
            bin_end = b;
            fprintf('[%2d-%2d) | %15.2f | %11.2f | %7.2f | %8.2f | %7d\n',...
                    bin_start, bin_end,...
                    results.mean_speed(b),...
                    results.mean_power(b),...
                    results.std_power(b),...
                    results.ci(b),...
                    results.count(b));
        else
            bin_start = b-1;
            bin_end = b;
            fprintf('[%2d-%2d) | %15s | %11s | %7s | %8s | %7d*\n',...
                    bin_start, bin_end,...
                    'NaN','NaN','NaN','NaN',results.count(b));
        end
    end
    fprintf('\n*Insufficient samples (<%d)\n', MIN_SAMPLES_PER_BIN);
    
    % Calculate and display key bin statistics explicitly
    fprintf('\n--- Coursework Required Metrics Verification ---\n');
    fprintf('1. Mean wind speed: Calculated for all valid bins\n');
    fprintf('2. Mean energy production: Calculated for all valid bins\n');
    fprintf('3. Standard deviation: Calculated for all valid bins\n');
    fprintf('4. 95%% Confidence intervals: z=%.2f × σ/√n\n', norminv(0.975));
    
    % Focus on specific bin (e.g., 15m/s) for detailed explanation
    idx = 16; % For bin 15-16 m/s
    if ~isnan(results.mean_speed(idx))
        fprintf('\nExample calculation for [15-16) m/s bin:\n');
        fprintf('a. Mean wind speed = %.2f m/s\n', results.mean_speed(idx));
        fprintf('b. Mean energy = %.2f kWh/10min\n', results.mean_power(idx));
        fprintf('c. Standard deviation = %.2f kWh/10min\n', results.std_power(idx));
        fprintf('d. 95%% CI = %.2f ± %.2f kWh/10min (%.2f × %.2f/√%d)\n', 
                results.mean_power(idx), results.ci(idx), 
                norminv(0.975), results.std_power(idx), sqrt(results.count(idx)));
    end
    fprintf('\n');
end
