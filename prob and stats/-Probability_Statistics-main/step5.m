...
% Create R² table with the exact required format
R2_table = table(...
    ...
    'VariableNames', {'R2_calculation', 'Cement_Transformed', 'Cement_Raw', 'Binder_Transformed', 'Binder_Raw'});

% Display with better formatting
disp('R² Results:')
disp('                     Cement case                    Binder case')
disp('R2 calculation    Transformed    Raw data       Transformed    Raw data')
disp('----------------------------------------------------------------')
fprintf('Training data      %.4f       %.4f          %.4f       %.4f\n', ...
    R2_trans_cem_train, R2_raw_cem_train, R2_trans_bind_train, R2_raw_bind_train);
fprintf('Testing data       %.4f       %.4f          %.4f       %.4f\n', ...
    R2_trans_cem_test, R2_raw_cem_test, R2_trans_bind_test, R2_raw_bind_test);

% Plot residual densities (raw scale only)
figure('Position', [100, 100, 1000, 400]);

% Cement Case (first plot)
subplot(1,2,1);
...
title('Raw Residual Distributions');
xlabel('Residual (MPa)');
ylabel('Density');
legend('Training', 'Testing');
grid on;

% Binder Case (second plot)
subplot(1,2,2);
...
sgtitle('Raw Residual Distributions');
set(gcf, 'Color', 'white');

% Print some statistics
fprintf('\nResidual Statistics (MPa):\n');
fprintf('Cement Case:\n');
fprintf('  Training: Mean = %.2f, Std = %.2f\n', mean(train_res_raw_cem), std(train_res_raw_cem));
fprintf('  Testing:  Mean = %.2f, Std = %.2f\n', mean(test_res_raw_cem), std(test_res_raw_cem));
fprintf('Binder Case:\n');
fprintf('  Training: Mean = %.2f, Std = %.2f\n', mean(train_res_raw_bind), std(train_res_raw_bind));
fprintf('  Testing:  Mean = %.2f, Std = %.2f\n', mean(test_res_raw_bind), std(test_res_raw_bind));
